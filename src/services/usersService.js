...
export default {
 AllUsers(credantials) {
  ...
  return Api().get("/admin/accounts?page="+page+"&size="+size+"&search="+search+"&filter="+filter,{
      headers: {
  ...
  toogleAccount(credantials) {
    ...
    let link = credantials.state == false ? "admin/client/activate" : "admin/client/deactivate";
